...
void fillInteractionTree(const simb::MCParticle*, const Vertex&, const std::map<int, const simb::MCParticle*>&, TTree*, float&, float&, float&, float&, float&,
                         float&, float&, float&, int&, std::vector<float>&, std::vector<float>&, std::vector<float>&, std::vector<float>&, std::vector<float>&,
                         std::vector<float>&, std::vector<float>&, std::vector<float>&, std::vector<int>&);
...
void getDescendants(int, const std::vector<int>&, const std::vector<int>&, const std::map<int, const simb::MCParticle*>&,
                    std::vector<const simb::MCParticle*>&);

std::string pdgToName(int pdgCode);
...
class MyEnergyAnalysis : public art::EDAnalyzer {
 public:
  ...
  struct Config {
    ...
    fhicl::Atom<art::InputTag> GenieGenModuleLabel{Name("GenieGenModuleLabel"), Comment("tag of the input data product with the event generator information")};

    fhicl::Atom<art::InputTag> SimulationLabel{Name("SimulationLabel"), Comment("tag of the input data product with the detector simulation information")};
...
void MyEnergyAnalysis::analyze(const art::Event& event) {
    ...
    for (int p = 0; p < mclist[0]->NParticles(); p++) {
      ...
      if (fP_StatusCode.at(p) == 1)  // Stable Final State
      {
        ...
        } else if (fP_PDG.at(p) == 111)  // kPdgPi0
        {
          ePi0 += fP_Ek.at(p);
          nPi0++;
        } else if (fP_PDG.at(p) == 321 || fP_PDG.at(p) == -321 || fP_PDG.at(p) == 311 || fP_PDG.at(p) == -311 || fP_PDG.at(p) == 130 || fP_PDG.at(p) == 310 ||
                   fP_PDG.at(p) == 22 || (fP_PDG.at(p) >= 100 && fP_PDG.at(p) <= 9999) ||
                   (fP_PDG.at(p) >= -9999 && fP_PDG.at(p) <= -100))  // kPdgKP, kPdgKM, kPdgK0, kPdgAntiK0, kPdgK0L, kPdgK0S, kPdgGamma, IsHadron(pdg)
        {
          eOther += fP_Ek.at(p);
          nOther++;
        }
  ...
  if (debug) std::cout << " : Checking primary particles - Number of primaries in this event: " << fSim_nParticles << std::endl;
  for (int i = 0; i < fSim_nParticles; i++) {

    if(debug) {
      
      std::cout << std::fixed << std::setprecision(4);
      std::cout << "    Collecting info from SimParticles[" << i << "]: " << 
    pdgToName(fSimP_PDG_vec[i])<< ",\t TrackID: " << fSimP_TrackID_vec[i] << ", Mother: " << fSimP_Mom_vec[i] << ", StatusCode: " << fSimP_SC_vec[i] << ", Vtx: ("<< fSimP_vtx_x_vec[i] << "," << fSimP_vtx_y_vec[i] << "," << fSimP_vtx_z_vec[i] << "), Ptot: " << fSimP_ptot_vec[i] << ", P:(" << fSimP_px_vec[i] << "," << fSimP_py_vec[i] << "," << fSimP_pz_vec[i]<< "), E: " << fSimP_E_vec[i] << ", Mass: " << fSimP_M_vec[i] << ", KE: " << fSimP_Ek_vec[i] << std::endl;
  ...
  if (debug) std::cout << " : Collecting Daughters of each primary particle" << std::endl;
  for (size_t i = 0; i < fSimP_TrackID_vec.size(); i++) {
    ...
    if (debug) std::cout << "  > Processing primary particle: " << fSimP_TrackID_vec[i] << " out of " << fSimP_TrackID_vec.size()<<",\t TrackID: " << fSimP_TrackID_vec[i] << ", Mother: " << fSimP_Mom_vec[i] << ", StatusCode: " << fSimP_SC_vec[i] << ", Vtx: ("<< fSimP_vtx_x_vec[i] << "," << fSimP_vtx_y_vec[i] << "," << fSimP_vtx_z_vec[i] << "), Ptot: " << fSimP_ptot_vec[i] << ", P:(" << fSimP_px_vec[i] << "," << fSimP_py_vec[i] << "," << fSimP_pz_vec[i]<< "), E: " << fSimP_E_vec[i] << ", Mass: " << fSimP_M_vec[i] << ", KE: " << fSimP_Ek_vec[i] <<", Number of Interaction Vertices: " << interactionVertices.size() << std::endl;
    
    for (const Vertex& vtx : interactionVertices) {
      fillInteractionTree(currentpart, vtx, particleMap, fInteractionTree, fInX, fInY, fInZ, fInT, fInPx, fInPy, fInPz, fInE, fInPDG, fOutX, fOutY, fOutZ,
                          fOutT, fOutPx, fOutPy, fOutPz, fOutE, fOutPDG);
    }
    if (currentMom == 0) {
    ...
    if (primary_particle != "8") {  // Exclude neutron interactions for now
      for (int j = 0; j < trkIDsize; j++) {
        if (fSimP_Mom_vec[j] == fSimP_TrackID_vec[k]) {  // Loop through particles again to see which particles are tagged with primary as mom
          ...
          //	if(primary_position.X() == daughter_position_start.X() && primary_position.Y() == daughter_position_start.Y() && primary_position.Z() ==
          //daughter_position_start.Z()){ 	primary_end_energy = primary_momentum.E();	//Store Primary energy
    ...
    for (auto const& timeSlice : timeSlices) {
      ...
      for (auto const& energyDeposit : energyDeposits) {
        ...
        if (Wires[0].planeID().Plane == 0) {
          ...
          // std::cout << "Time Slice Number: " << timeSlice.first << "Energy Deposit TrackID: " << energyDeposit.trackID << "Energy Deposit Energy: "<<
          // energyDeposit.energy << std::endl;
          ...
          if (search != particleMap.end()) {  // found match in map
            ...
            // std::cout << "EDep MeV: "<< energyDeposit.energy << " from gamma: Mother trkid: " << pi0_trkID << ", E: " << particle.E() << ", mass: " <<
            ...
            }  // std::cout << "fire pip! pdg: "<< particle.PdgCode() << ", pip_trkID: " << pip_trkID << ", IsAncestorMotherPip: " <<
               // IsAncestorMotherPip(particle, pip_trkID, particleMap) << std::endl; } // end pip deposited energy
            ...
            else if (particle.PdgCode() == 321 || particle.PdgCode() == -321 || particle.PdgCode() == 311 || particle.PdgCode() == -311 ||
                     particle.PdgCode() == 130 || particle.PdgCode() == 310 || particle.PdgCode() == 22 ||
                     (particle.PdgCode() >= 100 && particle.PdgCode() <= -9999) ||
                     (particle.PdgCode() >= -9999 &&
                      particle.PdgCode() <= -100))  // eOther which includes: kPdgKP, kPdgKM, kPdgK0, kPdgAntiK0, kPdgK0L, kPdgK0S, kPdgGamma, IsHadron(pdg)
            {
  ...
  // Print out EDepMap
  // if ( false ) std::cout << "fGen_numu_E: "<< fGen_numu_E << ", fSim_mu_Edep_b2_debug: " << fSim_mu_Edep_b2_debug<< ", fSim_hadronic_Edep_b2_debug: " <<
  // fSim_hadronic_Edep_b2_debug << ", Tot had E track id: " << EDep_TrackID_vec.size() << std::endl;
  if (false) {
...
std::string pdgToName(int pdgCode) {
  switch (pdgCode) {
    ...
    // Nuclear fragments, 10LZZZAAAI
    case 1000180400: return "⁴⁰Ar";
    case 1000180380: return "³⁸Ar";
    case 1000180390: return "³⁹Ar";
    case 1000180370: return "³⁷Ar";
    case 1000080160: return "¹⁶O";
    case 1000080170: return "¹⁷O";
    case 1000080180: return "¹⁸O";
    case 1000010020: return "²H";
    case 1000160350: return "³⁵S";
    case 1000170390: return "³⁹Cl";
    case 1000170370: return "³⁷Cl";
...
void fillInteractionTree(const simb::MCParticle* incoming, const Vertex& vertex, const std::map<int, const simb::MCParticle*>& particleMap,
                         TTree* fInteractionTree, float& fInX, float& fInY, float& fInZ, float& fInT, float& fInPx, float& fInPy, float& fInPz, float& fInE,
                         int& fInPDG, std::vector<float>& fOutX, std::vector<float>& fOutY, std::vector<float>& fOutZ, std::vector<float>& fOutT,
                         std::vector<float>& fOutPx, std::vector<float>& fOutPy, std::vector<float>& fOutPz, std::vector<float>& fOutE,
                         std::vector<int>& fOutPDG) {
  if (debug)
    std::cout << "     - Incoming particle: " << pdgToName(incoming->PdgCode()) << ", TrackID: " << incoming->TrackId() << ", Number of trajectory points: " << incoming->NumberTrajectoryPoints() << std::endl;

  // These are given in input but should not - so we reinitialize them
  fOutX.clear();
  fOutY.clear();
  fOutZ.clear();
  fOutT.clear();
  ...
  if (debug) 
    std::cout << "       - Vertex position: (" << vertex.x << ", " << vertex.y << ", " << vertex.z << "), Time: " << vertex.t << std::endl;

  for (unsigned int i = 0; i < incoming->NumberTrajectoryPoints(); ++i) {
    if (debug && i%10 == 1) std::cout<< "         - Checking trajectory point " << i << ": Position: (" << incoming->Position(i).X() << ", " << incoming->Position(i).Y() << ", " << incoming->Position(i).Z() << ")" << std::endl;

    TLorentzVector pos = incoming->Position(i);

    double dist = std::sqrt(std::pow(pos.X() - vertex.x, 2) + std::pow(pos.Y() - vertex.y, 2) + std::pow(pos.Z() - vertex.z, 2));
    if (dist < minDist) {
      minDist = dist;
      bestMom = incoming->Momentum(i);
      // !!! WARNING: CHECK THE PRINT BELOW!!! What's in incoming->Momentum(i)???
      // std::cout<<incoming->Momentum(i)<<std::endl;
      // if (debug && incoming->Momentum(i)) std::cout << "       - i = " << i << ", bestMom = " << incoming->Momentum(i) << std::endl;
    }
  }
  ...
  for (const simb::MCParticle* daughter : vertex.daughters) {
    ...
    if (debug) std::cout << "       - Daughter particle: " << pdgToName(daughter->PdgCode()) << ", TrackID: " << dTrackID << std::endl;
...
void getDescendants(int motherID, const std::vector<int>& momVec, const std::vector<int>& TrkIDvec, const std::map<int, const simb::MCParticle*>& particleMap,
